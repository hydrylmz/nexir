...
fn main() {
    ...
    for major_ver in (8..=15).rev() {
        ...
        if ret == NV_ENC_SUCCESS {
            ...
            println!("NvEncodeAPICreateInstance SUCCEEDED with major version: {}", major_ver);
            break;
        }
    }
    ...
    println!("Test 1 (Original: struct_ver=0x{:X}, api_ver={}): ret = {}", orig_ver, orig_api, ret);
    ...
    for test_major in (8..=15).rev() {
        ...
        for struct_ver in 1..=2 {
            ...
            if ret == NV_ENC_SUCCESS {
                ...
                if ret != 4 && ret != 1 { // Filter out unsupported device/no device if possible
                    println!("Test (major={}, struct_ver={}): ret = {}", test_major, struct_ver, ret);
                }
            }
        }
    }
